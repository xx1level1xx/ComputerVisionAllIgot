...
int main(){
	...
	while(1){
		...
		//Mat temp2;
		normalize( tempMat, tempMat2, 0, 255, NORM_MINMAX, CV_32FC1, Mat() );
		//convertScaleAbs( dst_norm, dst_norm_scaled );
		numCorners=0;
		for( int i = 0; i < tempMat2.rows ; i++ )
		{
			for( int j = 0; j < tempMat2.cols; j++ )
			{
				if( (int) tempMat2.at<float>(i,j) > thresh )//tried to be the same
				{
	...
	while(1){
		...
		for(int i=0;i<numSelectedMedCorner;i++){
			...
			for(int j=0;j<count;j++){
				...
				if(square(cornerToMatchX[i]-realMedianX[j])<9&&square(cornerToMatchY[i]-realMedianY[j])<9){
					//cout<<"test2"<<endl;
					//cout<<"passed"<<endl;
					break;
				}
